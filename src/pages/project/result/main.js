            ...
            renderRow={(rowData, sectionID, rowID) => {
              return (
                <Item
                  arrow="horizontal"
                  onClick={() => {
                    this.props.history.push(`/project/detail?projectId=${rowData.projectId}`)
                  }}
                >
                  <div>{rowData.compName}</div>
                  <div className="mt12">{rowData.projectName}</div>
                  <Info className="mt12" label="甲方主体" value={rowData.firstParty} />
                  <Info className="mt12" label="乙方主体" value={rowData.secondParty} />
                </Item>)
            }}
            initialListSize={10}
            pageSize={10}
